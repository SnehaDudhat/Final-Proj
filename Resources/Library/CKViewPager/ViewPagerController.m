...
- (void)defaultSetup
{
    ...
    if (!self.tabsView) {
        ...
        if (self.shouldShowDivider) {
            ...
            [self.tabsView insertSubview:self.divider atIndex:0];
        }
    }
    ...
    if (self.shouldShowDivider) {
        CGFloat minDividerWidth = MAX(contentSizeWidth, self.view.frame.size.width);
        self.divider.frame = CGRectMake(0, self.tabHeight - self.dividerHeight, minDividerWidth, self.dividerHeight);
        self.divider.backgroundColor = self.dividerColor;
    }
